...
namespace pimoroni {
  PicoGraphics_PenInky7::PicoGraphics_PenInky7(uint16_t width, uint16_t height, IDirectDisplayDriver<uint16_t> &direct_display_driver)
  ...
  void PicoGraphics_PenInky7::frame_convert(PenType type, conversion_callback_func callback) {
    if(type == PEN_INKY7) {
      ...
      uint8_t buffer8[bounds.w];
      uint16_t buffer16[bounds.w];

      for(int32_t r = 0; r < bounds.h; r++)
      {
        driver.read_pixel_span(Point(0, r), bounds.w, buffer16);
        // for(int y=0; y < 800; y++)
        //   buffer[y] = rand() % 7;

        // convert byte storage to nibble storage
        uint16_t *pDst = buffer16;
        uint16_t *pSrc = buffer16;
